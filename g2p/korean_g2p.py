...
def encode(graph, rulebook='./g2p/rulebook.txt'):
    ...
    '''
    for i in range(len(encoded_prono)):
        encoded_prono[i] = encoded_prono[i].to_list()
    '''
...
def decode(encoded_prono):
    ...
    for p in encoded_prono:
        ...
        '''
        for p in encoded_phone:
            if p < ONS.index('oh'):
                phone += ONS[p-1]
            elif ONS.index('oh') <= p and p < len(ONS):
                phone += ONS[p]
            elif len(ONS) <= p and p < len(ONS) + len(NUC):
                phone += NUC[p-len(ONS)]
            elif len(ONS) + len(NUC) < p:
                phone += RCD[p-(len(ONS)+len(NUC))]
        '''
